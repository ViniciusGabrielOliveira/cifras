/**
 * Seed script: populates the `acordes` Firestore collection with chord diagrams.
 * Run from the project root:
 *
 *   GOOGLE_APPLICATION_CREDENTIALS=/path/to/serviceAccountKey.json \
 *     node --require ../functions/node_modules/firebase-admin scripts/seed-acordes.js
 *
 * Or from inside functions/:
 *   GOOGLE_APPLICATION_CREDENTIALS=/path/to/serviceAccountKey.json \
 *     node -e "require('../scripts/seed-acordes.js')"
 *
...
async function seed() {
  console.log(`Seeding ${Object.keys(ACORDES).length} chords...`);
  const batch = db.batch();
  for (const [nome, variacoes] of Object.entries(ACORDES)) {
    const docId = nome.replace(/\//g, '_');
    const ref = db.collection('acordes').doc(docId);
    batch.set(ref, { variacoes });
    console.log(`  → acordes/${docId}`);
  }
  await batch.commit();
  console.log('Done!');
  process.exit(0);
